  ...
  $.extend($.jqx._jqxTabs.prototype, {
    ...
    defineInstance: function () {
      var settings = {
        ...
        _events: [
          ...
          'unselected', 'dragStart', 'dragEnd', 'locked', 'unlocked', 'collapsed', 'expanded', 'tabclick', 'dragEndAll'
        ],
        _initTabContentList: [],
    ...
    _uncapture: function (event) {
      ...
      if (mouseOverElementIndex === -1 || !event) {
        ...
        this._raiseEvent(10, {item: capturedIndex, dropIndex: mouseOverElementIndex, elem: this._capturedElement});
      ...
      var t = this;
      setTimeout(function () {
        t._raiseEvent(16, {item: capturedIndex, dropIndex: mouseOverElementIndex, elem: t._capturedElement});
      }, 100);
    },
    ...
    _prepareTabForDragging: function () {
      ...
      this._raiseEvent(9, {item: this._indexOf(this._capturedElement), element: this._capturedElement});
    },
    ...
    _moveSelectionTrack: function (item, duration, left) {
      ...
      if (this.selectionTracker && this._selectionTracker) {
        ...
        that._selectionTrackerHelper.animate({
          'top': headerPadding + topMargin - topOffset,
          'left': leftDisplacement + 'px',
          'height': parseInt(this._height(currentTitle) + itemPadding, 10),
          'width': that._width(currentTitle) + horizontalPadding
        }, duration, function () {
    ...
    _refreshBarPosition: function () {
      ...
      setTimeout(function () {
        ...
        if (that._selectedItem === -1) return;
    ...
    removeAt: function (index, notRemove) {
    ...
    addAt: function (index, title, content, fn) {
      if (index >= 0 && index <= this.length()) {
        ...
        if (fn) {
          $(navigatorInnerContainer).append(content)
        } else {
    ...
    addLast: function (title, content, fn) {
      this.addAt(this.length(), title, content, fn);
    },
