 ...
 */
Smart('smart-kanban', class Kanban extends Smart.DataView {
    // Kanban's properties.
    ...
    template() {
        ...
        return `<div id="container" role="presentation">
                    <div id="header" class="smart-data-view-header" role="toolbar">
                        <div id="customizeButton" class="smart-data-view-header-button smart-data-view-customize-button smart-unselectable"${tabindex} role="button" aria-expanded="false" aria-haspopup="dialog" aria-label="Customize tasks"><div role="presentation"></div></div>
                        <div id="filterButton" class="smart-data-view-header-button smart-data-view-filter-button smart-unselectable"${tabindex} role="button" aria-expanded="false" aria-haspopup="dialog" aria-label="Filter"><div role="presentation"></div></div>
                        <div id="sortButton" class="smart-data-view-header-button smart-data-view-sort-button smart-unselectable"${tabindex} role="button" aria-expanded="false" aria-haspopup="dialog" aria-label="Sort"><div role="presentation"></div></div>
                        <div id="searchButton" class="smart-data-view-header-button smart-data-view-search-button smart-unselectable"${tabindex} role="button" aria-expanded="false" aria-haspopup="dialog" aria-label="Search"></div>
                        <div id="headerDropDown" class="smart-data-view-header-drop-down smart-visibility-hidden" role="dialog">
                            ...
                            <div id="search" class="smart-data-view-search-box smart-hidden" role="presentation">
                                ...
                                <div id="searchPrev" class="smart-data-view-search-prev"${tabindex} role="button" aria-label="Previous"></div>
                                <div id="searchNext" class="smart-data-view-search-next"${tabindex} role="button" aria-label="Next"></div>
                                <div id="searchClose" class="smart-data-view-search-close"${tabindex} role="button" aria-label="Close search box"></div>
                            </div>
                        </div>
                    </div>
    ...
    render() {
        ...
        that._kanbanTaskMinWidth = parseFloat(computedStyle.getPropertyValue('--smart-kanban-task-min-width'));
        that._dataViewPadding = parseFloat(computedStyle.getPropertyValue('--smart-data-view-padding'));
        that._allColumns = [];
        that._customTags = [];
        that._selectedTasks = [];
    ...
    attached() {
        ...
        if (that.isCompleted && that._dialog) {
            ...
            document.body.appendChild(that._dialog);
        }
    }
    ...
    ensureVisible(task) {
        ...
        const parentScrollViewer = task.closest('smart-scroll-viewer');
    ...
    exportData(dataFormat, fileName, callback) {
        const that = this,
            ...
            dataExporter = new Smart.Utilities.DataExporter(),
    ...
    getState() {
        ...
        if (that._selectionInView) {
            selectionInColumn = that._selectionInView.closest('.smart-kanban-column').column.dataField;
    ...
    loadState(state) {
        ...
        if (!state) {
            state = window.localStorage.getItem('smartKanban' + that.id);
        ...
        Array.from(that.$.container.querySelectorAll('smart-scroll-viewer.smart-kanban-column-content-tasks')).forEach(scrollViewer => scrollViewer.clearContent());
        ...
        state.selection.selected.forEach(id => {
            const task = that.$.container.querySelector(`.smart-kanban-task[data-id="${id}"]`);
        ...
        if (state.selection.selectionStart) {
            that._selectionStart = that.$.container.querySelector(`.smart-kanban-task[data-id="${state.selection.selectionStart}"]`);
        }
        ...
        if (state.selection.selectionInColumn) {
            ...
            if (state.selection.swimlane) {
                that._selectionInView = columnElement.querySelector(`smart-scroll-viewer[swimlane=${state.selection.swimlane}]`);
            }
            else {
                that._selectionInView = columnElement.querySelector('smart-scroll-viewer');
            }
        }
        ...
        if (state.tabs) {
            ...
            that._allColumns.forEach(column => {
                if (column.selected !== undefined) {
                    ...
                    if (tabsHierarchy) {
                        columnElement.classList.toggle('smart-hidden', !selected);
    ...
    moveTask(task, newStatus) {
        ...
        const oldColumnScrollViewer = task.closest('smart-scroll-viewer'),
    ...
    removeTask(task, prompt) {
        ...
        if (!prompt) {
            const scrollViewer = task.closest('smart-scroll-viewer');
    ...
    saveState() {
        ...
        window.localStorage.setItem('smartKanban' + that.id, JSON.stringify(state));
    ...
    updateTask(task, newData) {
        ...
        if (newData.swimlane !== oldData.swimlane) {
            const columnElement = task.closest('.smart-kanban-column');

            if (that._hasSwimlane(columnElement.index)) {
                columnElement.querySelector(`smart-scroll-viewer[swimlane=${newData.swimlane}]`).appendChild(task);

                if (oldData.swimlane) {
                    that._refreshScrollViewer(columnElement.querySelector(`smart-scroll-viewer[swimlane=${oldData.swimlane}]`));
                }
            }
            else {
        ...
        that._refreshScrollViewer(task.closest('smart-scroll-viewer'));
        that._autoSaveState('dataSource');
    }
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            ...
            case 'animation':
            case 'theme': {
                let affectedElements = Array.from(that.$.container.querySelectorAll('smart-column-panel, smart-multi-column-filter-panel, smart-sort-panel'));

                if (dialog) {
                    affectedElements.push(dialog);

                    if (dialog.editPanelPopulated) {
                        affectedElements = affectedElements.concat(Array.from(dialog.$.container.querySelectorAll('.smart-element.editor')));
                    }
                }

                affectedElements.forEach(element => element[propertyName] = newValue);
                Array.from(that.$.container.getElementsByClassName('smart-kanban-task')).forEach(task => that._updateTaskColor(task));
                break;
            }
            case 'autoSaveState':
                that._autoSaveState();
            ...
            case 'formatStringDate':
                Array.from(that.$.container.getElementsByClassName('smart-kanban-task')).forEach(task => that._updateTaskDueDate(task));
            ...
            case 'messages':
                ...
                if (dialog) {
                    ...
                    if (dialog.editPanelPopulated) {
                        ...
                        dialog.$.container.querySelector('smart-button.add').title = that.localize('addSubtask');
            ...
            case 'tags':
            case 'textTemplate':
                Array.from(that.$.container.getElementsByClassName('smart-kanban-task')).forEach(task => that._renderTask(task));
                that._allColumns.forEach(column => that._refreshScrollViewer(column));
                break;
            case 'unfocusable':
                that._setFocusable();
            ...
            case 'users':
                that._closeList(that._userList, '_userSelectionFor');
                that._setUserList();
                Array.from(that.$.container.getElementsByClassName('smart-kanban-task')).forEach(task => that._updateUserIcon(task));
    ...
    _containerClickHandler(event) {
        ...
        const columnHeader = target.closest('.smart-kanban-column-header');

        if (columnHeader) {
            const column = columnHeader.parentElement.column;

            if (target.closest('.smart-kanban-column-header-add')) {
        ...
        const tab = target.closest('.smart-kanban-tab');
        ...
        if (tab && !tab.classList.contains('selected')) {
            ...
            previousSelection.columnElement.classList.add('smart-hidden');
            previousSelection.columnElement.column.selected = false;

            tab.classList.add('selected');
            tab.setAttribute('aria-selected', true);
            tab.columnElement.classList.remove('smart-hidden');
        ...
        const listItem = target.closest('.smart-kanban-list .item');
        ...
        if (target.closest('.smart-kanban-list.comments')) {
            ...
            if (sendButton) {
                ...
                if (!that._actionSelectionFor || that._actionSelectionFor !== comment && !that._actionSelectionFor.classList.contains('smart-kanban-task') &&
                    (!that._actionsList.parentElement || that._actionsList.classList.contains('smart-visibility-hidden'))) {
        ...
        const task = target.closest('.smart-kanban-task');

        if (!task) {
            return;
        }

        if (target.classList.contains('smart-kanban-task-user')) {
            if (that.userList && that._currentUser.allowEdit &&
                (!that._userSelectionFor || that._userSelectionFor !== task &&
                    (!that._userList.parentElement || that._userList.classList.contains('smart-visibility-hidden')))) {
        ...
        if (that.taskActions && target.classList.contains('smart-kanban-task-actions')) {
        ...
        if (that.taskComments && target.classList.contains('smart-kanban-task-comments')) {
            if (that.users.length > 0 &&
                (!that._commentSelectionFor || that._commentSelectionFor !== task &&
                    (!that._commentsList.parentElement || that._commentsList.classList.contains('smart-visibility-hidden')))) {
    ...
    _selectTask(event, task) {
        ...
        function selectSingleExtended() {
            ...
            that._selectionInView = task.closest('smart-scroll-viewer');
            that._autoSaveState('selection');
        ...
        if (selectionMode === 'zeroOrOne') {
            ...
            if (!event.ctrlKey && !event.shiftKey ||
                ...
                task.closest('smart-scroll-viewer') !== that._selectionInView) {
    ...
    _setUserList() {
        ...
        list.className = 'smart-kanban-list users smart-visibility-hidden';
    ...
    _openList(button, list, right) {
        ...
        list.classList.remove('smart-visibility-hidden');
    ...
    _closeList(list, flag) {
        ...
        function check() {
            if (!list.classList.contains('smart-visibility-hidden')) {
                return;
            }
        ...
        list.classList.add('smart-visibility-hidden');
        requestAnimationFrame(check);
    ...
    _setActionsList() {
        ...
        list.className = 'smart-kanban-list actions smart-visibility-hidden';
...
<div class="item" data-id="2" role="menuitem"><div class="name">${that.localize('remove')}</div></div>`;
    ...
    _setCommentsList() {
        ...
        list.className = 'smart-kanban-list comments smart-visibility-hidden';
        ...
        list.innerHTML = `<div class="smart-kanban-comments" role="list"></div>
<div class="smart-kanban-new-comment" role="presentation">
    <textarea placeholder="${that.localize('newComment')}"></textarea>
    <smart-button class="send primary" disabled${that._rtlAttr}>${that.localize('send')}</smart-button>
</div>`;

        const textarea = list.querySelector('textarea'),
            button = list.querySelector('smart-button');
    ...
    _createComment(comment) {
        ...
        if (!comment) {
            ...
            that._commentSelectionFor.querySelector('.smart-kanban-task-comments').setAttribute('num', data.comments.length || '');
        }
        else {
...
<div class="comment-main" role="presentation">
    <div class="comment-header" role="presentation">
        ...
        <div class="time" aria-label="Comment time">${comment.time ? new Smart.Utilities.DateTime(comment.time).toString(that.formatStringTime) : ''}</div>
    ...
    getTaskScrollViewer(columnElement, data) {
        ...
        if (that._hasSwimlane(columnElement.index)) {
            if (data.swimlane !== undefined && that.swimlanes.find(swimlane => swimlane.dataField === data.swimlane)) {
                scrollViewer = columnElement.querySelector(`smart-scroll-viewer[swimlane="${data.swimlane}"]`);
            }
            else {
                scrollViewer = columnElement.querySelector('smart-scroll-viewer');
    ...
    _applyTaskTemplate(task) {
        ...
        if (taskTags && typeof taskTags === 'string') {
            ...
            tagsContent = taskTags.map(tag => `<span class="smart-kanban-task-tag" role="listitem">${tag}</span>`).join('');
        }
        else {
            data.tags = '';
        }
        ...
        task.innerHTML = `<div class="smart-kanban-task-content" role="presentation">
    <div class="smart-kanban-task-text ${data.priority}">${text}</div>
    <div class="smart-kanban-task-user"${tabindex} role="button" aria-haspopup="listbox"></div>
</div>
...
<div class="smart-kanban-task-footer" role="toolbar" aria-label="Task footer">
    <div class="smart-kanban-task-tags" role="list" aria-label="Tags list">${tagsContent}</div>
    <div class="smart-kanban-task-actions"${tabindex} role="button" aria-haspopup="menu" aria-label="Open actions list"></div>
    <div class="smart-kanban-task-comments"${tabindex} num="${data.comments.length || ''}" role="button" aria-haspopup="dialog" aria-label="Open comments list"></div>
</div>`;
    ...
    _updateUserIcon(task) {
        const data = task.data,
            userIcon = task.getElementsByClassName('smart-kanban-task-user')[0];
    ...
    _updateTaskProgress(task) {
        const data = task.data,
            progressBar = task.querySelector('.smart-kanban-task-progress');
        ...
        progressBar.classList.toggle('bottom', data.checklist !== undefined);
    ...
    _updateTaskChecked(task) {
        const data = task.data,
            ...
            checked = task.querySelector('.smart-kanban-task-checked');
    ...
    _updateTaskDueDate(task) {
        const data = task.data,
            ...
            dateContainer = task.querySelector('.smart-kanban-task-due');

        if (dueDate) {
            dateContainer.innerHTML = new Smart.Utilities.DateTime(dueDate).toString(this.formatStringDate);
    ...
    _updateTaskColor(task) {
        const that = this,
            data = task.data,
            progressContainer = task.querySelector('.smart-kanban-task-progress-container'),
            progress = task.querySelector('.smart-kanban-task-progress'),
    ...
    _handleSwimlanes(update) {
        ...
        const columns = that.columns,
            headerSize = parseFloat(getComputedStyle(that).getPropertyValue('--smart-kanban-header-size')),
            container = that.$.container,
            containerRect = that.$.container.getBoundingClientRect(),
            columnsWithSwimlanes = that.$.container.querySelectorAll('.smart-kanban-column.has-swimlane');
        ...
        const firstColumnRect = firstColumn.getBoundingClientRect(),
            ...
            scrollViewers = Array.from(firstColumn.getElementsByTagName('smart-scroll-viewer'));
        ...
        swimlanes.forEach((swimlane, index) => {
            ...
            swimlaneElement.classList.toggle('smart-visibility-hidden', allColumnsCollapsed);
    ...
    _createColumnElements(siblingColumns, parent, container, level) {
        ...
        siblingColumns.forEach((column, index) => {
            ...
            if (column.columns) {
                ...
                innerHTML = innerHTML.replace('BBB', '<div class="smart-kanban-column-content-columns" role="presentation"></div>');
            }
            else {
    ...
    _createColumnElementsSwimlanes(siblingColumns, parent, container) {
        ...
        siblingColumns.forEach((column, index) => {
            ...
            let innerHTML =
                ...
                <div class="smart-kanban-column-content no-sub-columns" role="presentation"></div>`;
            ...
            if (!that._hasSwimlane(index)) {
                columnContentInnerHTML = `<smart-scroll-viewer class="smart-kanban-column-content-tasks"${that._rtlAttr}${that._tabindex} role="list"></smart-scroll-viewer>`;
            }
            else {
                let gridTemplateRows = [];

                for (let i = 0; i < swimlanes.length; i++) {
                    columnContentInnerHTML += `<smart-scroll-viewer class="smart-kanban-column-content-tasks" swimlane="${swimlanes[i].dataField}"${that._rtlAttr}${that._tabindex} role="list"></smart-scroll-viewer>`;
    ...
    _getColumnHeader(column, left, level = 1) {
        ...
        return `<div class="smart-kanban-column-header smart-unselectable"${tabindex} role="heading" aria-level=${level}>
                    <div class="smart-kanban-column-header-add"${tabindex} role="button" aria-label="Add new task"></div>
                    <div id="${that.id}ColumnHeaderLabel${column.dataField}" class="smart-kanban-column-header-label">${column.label}</div>
                    <div class="smart-arrow smart-arrow-${left ? 'left' : 'right'}" aria-hidden="true"></div>
                </div>`;
    }
    ...
    _refreshScrollViewer(column, force) {
        ...
        function refresh(scrollViewer, index) {
            ...
            if (column.collapsed) {
                ...
                requestAnimationFrame(() => scrollViewer.refresh());
        ...
        if (column instanceof Smart.ScrollViewer) {
            refresh(column);
            return;
        }

        if (this.swimlanes.length > 0) {
            Array.from(columnElement.querySelectorAll('smart-scroll-viewer')).forEach((scrollViewer, index) => refresh(scrollViewer, index));
        }
        else {
            refresh(columnElement.querySelector('smart-scroll-viewer'), 0);
        }
    }
    ...
    _containerDownHandler(event) {
        const that = this,
            ...
            task = target.closest('.smart-kanban-task');
    ...
    _containerKeydownHandler(event) {
        ...
        if (columnElement) {
            ...
            if (target.classList.contains('smart-kanban-column-content-tasks')) {
        ...
        if (that.$.headerDropDown.contains(target)) {
            if (key === 'Escape') {
                that.closePanel();
                that.$[/\s?([a-z]+)-panel/g.exec(kanbanHeaderDropDown.className)[1] + 'Button'].focus();
            }

            return;
        }

        if ((target.classList.contains('smart-data-view-header-button') ||
            target.closest('.smart-data-view-search-box') ||
            target.classList.contains('remove-button')) &&
            (key === 'Enter' || key === ' ')) {
            emulateClick();
        }

        if (target.classList.contains('smart-kanban-list')) {
            const list = target;
    ...
    _setFocusable() {
        ...
        elements = elements.concat(Array.from(
            container.querySelectorAll('.smart-kanban-column-header, .smart-kanban-column-header-add, .smart-kanban-tab, .smart-kanban-column-content-tasks, .smart-kanban-task-user, .smart-kanban-task-actions, .smart-kanban-task-comments')));
        ...
        if (that._dialog && that._dialog.editPanelPopulated) {
            ...
            Array.from(that._dialog.$.container.querySelectorAll('.smart-element.editor, smart-button.add, smart-button.ok, smart-button.cancel')).forEach(element => element.unfocusable = unfocusable);
    ...
    _startDragging(pressedTask, event) {
        ...
        const scrollViewer = pressedTask.closest('smart-scroll-viewer'),
        ...
        Smart.Kanban.kanbanTaskDragged = true;
        scrollViewer._scrollView.disableSwipeScroll = true;

        if (that.allowDrop) {
            Smart.Kanban.hoveredKanban = that;
            Smart.Kanban.hoveredItem = pressedTask;
        }
    }
    ...
    _documentMoveHandler(event) {
        ...
        function clear() {
            ...
            delete Smart.Kanban.kanbanTaskDragged;
            delete Smart.Kanban.hoveredKanban;
            delete Smart.Kanban.hoveredItem;
        ...
        if (Smart.Kanban.hoveredKanban) {
            clearInterval(Smart.Kanban.hoveredKanban._dragInterval);
            delete Smart.Kanban.hoveredKanban;
        }

        if (Smart.Kanban.hoveredItem) {
            Smart.Kanban.hoveredItem.classList.remove('drop-target');
            Smart.Kanban.hoveredItem.classList.remove('before', 'after');
            delete Smart.Kanban.hoveredItem;
        }
        ...
        const hoveredKanban = (that.shadowRoot ? realTarget.getRootNode().host : realTarget).closest('smart-kanban');

        if (!hoveredKanban || !hoveredKanban.allowDrop) {
            return;
        }

        const hoveredScrollViewer = realTarget.closest('.smart-kanban-column-content-tasks');

        Smart.Kanban.hoveredKanban = hoveredKanban;
        ...
        if (siblingTasks.length === 0 || siblingTasks.length === numberOfFiteredOut) {
            Smart.Kanban.hoveredItem = hoveredScrollViewer.$.scrollViewerContainer;
            Smart.Kanban.hoveredItem.classList.add('drop-target');
            return;
        }
        ...
        let hoveredItem = realTarget.closest('.smart-kanban-task');
        ...
        if (hoveredItem) {
            ...
            Smart.Kanban.hoveredItem = hoveredItem;
        }
    }
    ...
    _scrollViewerTouchmoveHandler(event) {
        ...
        if (target.closest('smart-scroll-viewer') && event.cancelable) {
            event.preventDefault();
    ...
    _documentUpHandler(event) {
        ...
        const targetKanban = Smart.Kanban.hoveredKanban,
            targetItem = Smart.Kanban.hoveredItem;
    ...
    _addDragFeedback() {
        ...
        feedback.className = 'smart-kanban-feedback';
        ...
        ['rightToLeft', 'taskActions', 'taskComments', 'taskDue', 'taskPriority', 'taskProgress', 'taskTags', 'taskUserIcon'].forEach(propertyName => {
            if (that[propertyName]) {
                feedback.setAttribute(Smart.Utilities.Core.toDash(propertyName), '');
            }
        });
        ...
        if (numberOfDraggedItems > 1) {
            ...
            additionalTask.className = 'smart-kanban-feedback-additional';
    ...
    _moveDraggedItems(dragDetails) {
        ...
        let dropDetails = dragDetails.DropDetails,
            ...
            dropColumn = dropTarget.closest('.smart-kanban-column'),
            originScrollViewer = dragDetails.Item.closest('smart-scroll-viewer'),
            columnObject = dropColumn.column;
        let dropParent;

        if (dropTarget.classList.contains('smart-kanban-task')) {
        ...
        dropParent = dropParent.closest('smart-scroll-viewer');
    ...
    _getCurrentDataSource() {
        const allTasks = Array.from(this.$.container.getElementsByClassName('smart-kanban-task')),
            currentDataSource = [];
    ...
    _validateTaskArgument(task) {
        ...
        if (task instanceof HTMLElement && that.$.container.contains(task) && task.classList.contains('smart-kanban-task')) {
            return task;
        }
        else if (task !== null && (typeof task === 'number' || typeof task === 'string')) {
            return that.$.container.querySelector(`.smart-kanban-task[data-id="${task}"]`);
        }
    }
    ...
    _createDialog() {
        ...
        footerTemplate.innerHTML = `<smart-button class="ok primary"${that._rtlAttr}>${that.localize('ok')}</smart-button>
<smart-button class="cancel"${that._rtlAttr}>${that.localize('cancel')}</smart-button>`;
        ...
        dialog.className = 'smart-kanban-window';
...
<div class="edit"></div>`;
    ...
    _dialogClickHandler(event) {
        ...
        if (target.closest('.ok')) {
            ...
            dialog.editors.checklist.removeChild(target.closest('smart-list-item'));
        }
    }
        ...
        if (dialog.ok) {
            ...
            if (dialog.classList.contains('prompt')) {
                ...
                if (commentParentTask) {
                    ...
                    commentParentTask.querySelector('.smart-kanban-task-comments').setAttribute('num', data.comments.length || '');
    ...
    _populateEditPanel() {
        ...
        checklistTemplate.innerHTML = `<div class="smart-kanban-sub-task"><span class="label">{{label}}</span><span class="remove-subtask" aria-label="Remove subtask" title="${that.localize('removeSubtask')}"></span></div>`;
...
<smart-combo-box class="editor tags-editor" selection-mode="zeroOrMany" selection-display-mode="tokens"${sync} aria-labelledby="${id}LabelTags"></smart-combo-box>
<div class="editor-container">
    ...
    <smart-input class="editor status-editor" data-source='${JSON.stringify(statusSource)}' drop-down-button-position="right" readonly${sync} aria-labelledby="${id}LabelStatus"></smart-input>
    <smart-input class="editor swimlane-editor" data-source='${JSON.stringify(swimLanesSource)}'${swimLanesSource.length === 0 ? ' disabled' : ''} drop-down-button-position="right" readonly${sync} aria-labelledby="${id}LabelSwimlane"></smart-input>
</div>
<div class="editor-container">
    ...
    <smart-input class="editor asignee-editor" data-source='${JSON.stringify(asigneeSource)}'${asigneeSource.length === 0 ? ' disabled' : ''} drop-down-button-position="right"${sync} aria-labelledby="${id}LabelAssignedTo"></smart-input>
    <smart-numeric-text-box class="editor progress-editor" input-format="floatingPoint" min="0" max="100" show-unit unit="%"${sync} aria-labelledby="${id}LabelProgress"></smart-numeric-text-box> 
</div>
<div class="editor-container">
    ...
    <smart-date-time-picker class="editor start-date-editor" calendar-button drop-down-append-to="body" drop-down-display-mode="calendar" format-string="${that.formatStringDate}" nullable${sync} aria-labelledby="${id}LabelStartDate"></smart-date-time-picker>
    <smart-date-time-picker class="editor due-date-editor" calendar-button drop-down-append-to="body" drop-down-display-mode="calendar" format-string="${that.formatStringDate}" nullable${sync} aria-labelledby="${id}LabelDueDate"></smart-date-time-picker>
</div>
<div class="editor-container">
    ...
    <smart-input class="editor priority-editor" data-source='${JSON.stringify(prioritySource)}' drop-down-button-position="right" readonly${sync} aria-labelledby="${id}LabelPriority"></smart-input>
    <smart-color-input class="editor color-editor" drop-down-button-position="right"${sync} aria-labelledby="${id}LabelColor"></smart-color-input>
</div>
...
<div class="new-container">
    ...
    <smart-button class="add primary" title="${that.localize('addSubtask')}" aria-label="Add subtask"${sync}>+</smart-button>
</div>
<smart-list-box class="editor checklist-editor" item-template="${checklistTemplate.id}" selection-mode="checkBox"${sync} aria-labelledby="${id}LabelChecklist"></smart-list-box>`;
    ...
    _endEdit() {
        ...
        if (editors.status.$.input.dataValue) {
            newStatus = editors.status.$.input.dataValue;
        }
    ...
    _autoSaveState(toUpdate) {
        ...
        window.localStorage.setItem('smartKanban' + that.id, JSON.stringify(that._autoSavedState));
    }
    ...
    _applyFilter(filters, operator) {
        ...
        const allTasks = Array.from(that.$.container.getElementsByClassName('smart-kanban-task'));

        for (let i = 0; i < dataSource.length; i++) {
            const task = allTasks[i],
                filteredOut = !dataSource[i].$.filtered;

            task.classList.toggle('smart-hidden', filteredOut);
    ...
    _applySort() {
        ...
        that._allColumns.forEach(column => {
            const columnElement = that._columnToElement.get(column),
                tasksParent = columnElement.querySelector('.smart-scroll-viewer-content-container'),
    ...
    _closeSearchPanel() {
        ...
        if (that._searchInfo) {
            Array.from(that.$.container.querySelectorAll('.smart-kanban-task.smart-data-view-found, .smart-kanban-task.smart-data-view-highlighted'))
                .forEach(task => task.classList.remove('smart-data-view-found', 'smart-data-view-highlighted'));
    ...
    _search(query, highlight = true) {
        ...
        const source = new Smart.DataAdapter({
        ...
        Array.from(that.$.container.getElementsByClassName('smart-kanban-task')).forEach(task => {
            ...
            if (foundIdsObject[id]) {
                ...
                if (highlight && foundIdsArray[0] === id) {
                    task.classList.add('smart-data-view-highlighted');
                }

                task.classList.add('smart-data-view-found');
            }
        });
