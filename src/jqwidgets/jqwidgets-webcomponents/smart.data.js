...
class DataAdapter {
    constructor(config) {
        ...
        that.$document = Smart.Utilities.Extend(document);
        that.boundSource = false === that.observable ? [] : new Smart.ObservableArray();
        that.dataItemById = [];
    ...
    dataBind() {
        ...
        const completed = () => {
            ...
            if (!that.groupBy) {
                ...
                if (that.groupBy.toArray) {
                    that.groupBy = that.groupBy.toArray();
                }
            }

            that.groupBy = new Smart.ObservableArray(that.groupBy);
    ...
    onVirtualDataSourceRequested(requestCallback, details) {
        ...
        if (that.virtualDataSource) {
            const getDataSource = function (dataAdapterSettings) {
                if (dataAdapterSettings.virtualDataSourceLength !== undefined) {
    ...
    update(index, dataSourceItem) {
        ...
        if (Smart.Utilities.Types.isArray(index) && Smart.Utilities.Types.isArray(dataSourceItem)) {
    ...
    clear() {
        ...
        that._cachedValues = [];
        that.boundSource = that.observable ? new Smart.ObservableArray() : [];
        that.dataItemById = [];
        that.refreshHierarchy();
    }
    ...
    _bindToArray() {
        ...
        that.boundSource = false === that.observable ? dataArray : new Smart.ObservableArray(dataArray);
    ...
    _createFilter(dataType, filterExpressions) {
        ...
        for (let i = 0; i < filterExpressionsArray.length; i++) {
            ...
            if (filterExpression.length > 1) {
                const filterSubGroup = new Smart.FilterGroup();
    ...
    _filter(filters, operator = 'and') {
        ...
        for (let i = 0; i < filters.length; i++) {
            ...
            if (filter[1] instanceof Smart.FilterGroup) {
    ...
    _sort(dataSource, sortColumns, directions, dataTypes, customSortingCallback) {
        ...
        if (dataSource && dataSource.constructor && dataSource instanceof Smart.ObservableArray) {
        ...
        const getCompareFunction = function (a, knownDataType) {
            ...
            switch (dataType) {
                ...
                case 'dateTime':
                    ...
                    else if (a instanceof Smart.Utilities.DateTime ||
                        a instanceof Smart.Utilities.BigNumber) {
                        compareFunction = function (a, b) {
                            return a.compare(b);
                        };
                    }
                ...
                case 'object':
                    ...
                    else if (a instanceof Smart.Utilities.Complex || (window.NIComplex && a instanceof window.NIComplex)) {
                        const complexNumericProcessor = new Smart.Utilities.ComplexNumericProcessor();
    ...
    filter(filterColumns, filterGroups, customFilteringCallback) {
        Smart.DataAdapter.Filter(this.boundSource, filterColumns, filterGroups, customFilteringCallback);
    }

    sort(sortColumns, directions, customSortingCallback) {
        Smart.DataAdapter.Sort(this.boundSource, sortColumns, directions, customSortingCallback);
    }
...
Smart.DataAdapter = window.smartDataAdapter = DataAdapter;
