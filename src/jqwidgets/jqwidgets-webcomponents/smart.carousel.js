...
Smart('smart-carousel', class Carousel extends Smart.ContentElement {
    ...
    */
    get enableShadowDOM() {
        ...
        //NOTE: CardView flag to avoid ShadowDOM creation
    ...
    */
    ...
    /**
     * Called when the element and it's styles are rendered. After ready
     */
    render() {
        const that = this;

        that.setAttribute('role', 'region');
        that.setAttribute('aria-roledescription', 'carousel');
    ...
    */
    _handle3dMode(newIndex) {
        ...
        for (let i = 0; i < itemsCount; i++) {
            const currentPosition = (i - newIndex) * (that.rightToLeft ? -1 : 1);
    ...
    */
    _handleArrowsActiveState(newIndex/*, oldIndex*/) {
        ...
        if (that.rightToLeft) {
            that.$.arrowRight.disabled = newIndex === 0;
            that.$.arrowLeft.disabled = newIndex === (itemsCount - 1);
        }
        else {
            that.$.arrowLeft.disabled = newIndex === 0;
            that.$.arrowRight.disabled = newIndex === (itemsCount - 1);
        }
    }
    ...
    */
    _handleIndicatorsState(from, to) {
        ...
        if (from !== to) {
            ...
            that._indicators[from].setAttribute('aria-selected', false);
        }

        that._indicators[to].classList.add('smart-active');
        that._indicators[to].setAttribute('aria-selected', true);
    }
    ...
    */
    _handleKeyDown(event) {
        ...
        switch (key) {
            case 'ArrowLeft':
            case 'ArrowDown':
                that.rightToLeft ? that.next() : that.prev();
                break;
            case 'ArrowUp':
            case 'ArrowRight':
                that.rightToLeft ? that.prev() : that.next();
                break;
            case 'Home':
                that._goToItem(0);
                break;
    ...
    */
    _generateIndicator(id) {
        ...
        indicatorContainer.setAttribute('role', 'tab');
        indicatorContainer.setAttribute('aria-selected', false);
    ...
    */
    _generateItem(id) {
        const that = this,
            itemId = id ? id : 0,
            item = that.dataSource[itemId],
            itemContainer = document.createElement('div');

        itemContainer.id = that.id + 'Slide' + id;
        ...
        if (that.itemTemplate) {
            ...
            if (typeof item === 'string') {
                ...
                itemContainer.innerHTML = `<div class="smart-carousel-item-container" style="background-image:url('${item.image || ''}')" role="presentation">
                        <h2 id="${itemContainer.id + 'Label'}" class="smart-carousel-item-label">${item.label || ''}</h2 >
        ...
        if (item.label) {
            itemContainer.setAttribute('aria-labelledby', itemContainer.id + 'Label');
            that._indicators[id].setAttribute('aria-labelledby', itemContainer.id + 'Label');
        }
        else {
            itemContainer.setAttribute('aria-label', 'Slide ' + id);
            that._indicators[id].setAttribute('aria-label', 'Slide ' + id);
        }

        that._indicators[id].setAttribute('aria-controls', itemContainer.id);
        itemContainer.setAttribute('role', 'tabpanel');
