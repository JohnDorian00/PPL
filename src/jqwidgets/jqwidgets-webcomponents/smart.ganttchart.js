...
Smart('smart-gantt-chart', class GanttChart extends Smart.ScrollViewer {
    ...
    */
    ...
    attached() {
        ...
        if (!that._scrollView) {
            that._scrollView = new Smart.Utilities.Scroll(that.$.taskSplitterItem, that.$.horizontalScrollBar, that.$.verticalScrollBar);
        }

        if (!that._resourceScrollView && that.$.mainSplitter.contains(that.$.resourceSplitter)) {
            that._handleResourceTreeEvents(true);
            that._resourceScrollView = new Smart.Utilities.Scroll(that.$.resourceSplitterItem, that.$.horizontalScrollBar, that.$.resourceVerticalScrollBar);
        }
    ...
    */
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            ...
            case 'rightToLeft':
                ...
                Array.from(that.querySelectorAll('smart-splitter, smart-tree, smart-scroll-bar')).forEach(el => el[propertyName] = newValue);
                that.closeWindow();
                that._handleInverted();
            ...
            case 'yearFormat':
                ...
                //that._tasks.forEach(task => that._refreshTreeColumnsData(task));
                break;
            ...
            case 'treeSize': {
                ...
                //Resource Timeline
                if (that.$.mainSplitter.contains(that.$.resourceSplitter)) {
                    that.$.resourceTreeItem[splitterPropName] = newValue;
                    that.$.resourceTreeItem.refresh();
                }
                break;
            }
            case 'taskPanelMin':
    ...
    render() {
        ...
        //that._predefPropertyGetter('resources');
   ...
   */
    ...
    beginUpdate() {
        ...
        that._isUpdating = { started: true, type: {} };
    }
    ...
    clearTasks() {
        ...
        that.$.container.style.removeProperty('--smart-gantt-chart-task-timeline-content-height');
    ...
    clearResources() {
        ...
        for (let i = 0; i < treeSplitterItems.length; i++) {
            ...
            if (splitterItem === that.$.resourceTreeSplitterItem) {
                ...
                const dataContainer = splitterItem.getElementsByClassName('smart-resource-tree-content')[0];
    ...
    exportData(dataFormat, callback) {
        ...
        if (!dataExporter.style) {
            const computedStyle = window.getComputedStyle(that),
                ...
                header = {
                    height: that.$[`${type}TreeSplitter`].querySelector(`.smart-${type}-tree-header`).offsetHeight + 'px',
            ...
            for (let i = 0; i < columnsData.length; i++) {
                const column = columnsData[i],
                    ...
                    row = treeSplitterItems[i].querySelector('.smart-tree-item-label-container') || treeSplitterItems[i].querySelector(`.smart-${type}-label-container`);
    ...
    getState() {
        const that = this,
            tasks = that._getTasksJSON(that.view !== 'resource' && that._view !== undefined) || [];

        return {
            tasks: JSON.parse(JSON.stringify(tasks)),
            selectedIndexes: that.selectedIndexes.slice(0),
            resources: JSON.parse(JSON.stringify(that._resources.slice(0)))
        };
    }
    ...
    */
    get tasks() {
        ...
        return that._tasks.map(function (task) {
            return {
                id: task.id,
                class: task.class,
                ...
                label: task.label,
                value: task.value,
                connections: task.connections.slice(0),
                ...
                disableResize: task.disableResize,
                disableDrag: task.disableDrag,
                disableResources: task.disableResources,
                progress: task.progress,
                assignedTo: task.project ? { id: task.project.id, index: that._tasks.indexOf(task.project), label: task.project.label } : undefined,
                path: that._getItemPath(task)
            }
        });
    }
   ...
   */
    get resources() {
        ...
        function getTasksAssigned(resource) {
            let assignedTo = [];
        ...
        return that._resources.map(function (resource) {
            return {
                id: resource.id,
                class: resource.class,
                assignedTo: getTasksAssigned(resource),
                label: resource.label,
                value: resource.value, //The quantity of the resource
                type: resource.type, //Custom usage
                hidden: resource.hidden, // Determines whether the resource is dispalyed in the Resource Panel or not
                //formatFunction: () => //Custom format function for the Timeline label
                //nonworkingDays: [] //Custom working time, per days
                capacity: resource.capacity, //Working capacity (hours): 0-24
                maxCapactiy: resource.maxCapacity,//Maximum working capacity
                minCapacity: resource.minCapacity,//Minimum working capacity
                progress: resource.progress, // Note: Working progress 0-100,
     ...
     */
    getTaskIndex(task) {
        return this._getItemIndex(task, 'task');
    }
    ...
    */
    ...
    _getItemIndex(item, type) {
        ...
        if (item instanceof Smart.TreeItem || item instanceof Smart.TreeItemsGroup) {
            return [].slice.call(that.$.taskTree.querySelectorAll('smart-tree-item, smart-tree-items-group')).indexOf(item);
        }
        ...
        if (item instanceof HTMLElement && item.classList.contains('smart-timeline-task')) {
            return items.index(item[`_${type}`]);
        }
    ...
    getTaskPath(task) {
        ...
        if (task instanceof HTMLElement && task.classList.contains('smart-timeline-task') && that.$.mainSplitter.contains(task)) {
    ...
    */
    ...
    loadState(state) {
        ...
        const dataSource = JSON.parse(JSON.stringify(state.tasks)),
            selectedIndexes = JSON.parse(JSON.stringify(state.selectedIndexes)),
    ...
    */
    ...
    insertTask(index, newTask) {
        ...
        that._insertNewTreeItem(index, newTask, insertAsLastItem, 'task');
        ...
        for (let i = 0; i < newTasks.length; i++) {
            ...
            that._refreshTaskResources(task);
        }
        ...
        that.$.container.style.setProperty('--smart-gantt-chart-task-timeline-content-height', that.$.taskTimelineCellsContainer.offsetHeight + 'px');

        that._select('task', newTasks[0]);

        delete that._taskAPIManipulation;

        that._refreshTaskResources(that._refreshTaskResources(targetTask));
    }
    ...
    openWindow(target) {
        ...
        if (typeof target === 'string') {
            if (target.indexOf('.') > -1) {
                that._openPopupWindow(that.$.timelineTasksContainer.children[that._getTaskIndexById(that._getItemByTreeIndex('task', target))]);
                return;
            }
    ...
    addTaskTo(task, project) {
        ...
        if (typeof project === 'string') {
            ...
            if (!project) {
                project = that._getItemByTreeIndex('task', project);
            }
        }
        ...
        that.insertTask(that._getItemPath(project) + '.' + project.tasks.length, task);
    }
    ...
    updateTask(index, taskDetails) {
        ...
        if (typeof index === 'string') {
            ...
            if (!task) {
                index = that._tasks.indexOf(that._getItemByTreeIndex('task', index));
                task = tasks[index];
            }
            else {
                index = tasks.indexOf(task);
        ...
        if (typeof index === 'number') {
            task = tasks[index] || tasks.find((task) => task.id && task.id.toString() === index.toString());
        }
        else if (typeof index === 'object') {
            task = tasks[that._getItemIndex(index)];
            index = tasks.indexOf(task);
        }
                    ...
                    if (refereshTaskTree) {
                        ...
                        for (let i = 0; i < taskTreeSplitterItems.length; i++) {
                            ...
                            if (that.$.taskTreeSplitterItem === splitterItem) {
                                ...
                                if (taskDetails[prop] === 'project' && !targetTaskItem.lastElementChild.classList.contains('smart-task-drop-down')) {
                                    const dropDownContainer = document.createElement('div');

                                    dropDownContainer.classList.add('smart-task-drop-down', 'smart-visibility-hidden');
                                    targetTaskItem.appendChild(dropDownContainer);
                                }
                                else if (targetTaskItem.lastElementChild.classList.contains('smart-task-drop-down')) {
        ...
        if (!that.selectedIndexes.includes(index)) {
            //Select the new Task
            that._taskAPIManipulation = true;
    ...
    removeTask(index) {
        const that = this;
        let tasks = that._tasks.slice(0), targetTask;
        ...
        if (typeof index === 'string') {
            ...
            if (!targetTask) {
                ...
                if (index !== that._getItemPath(targetTask)) {
                    return;
                }
            }
        ...
        if (targetTask.type === 'project') {
            ...
            for (let i = targetTasks.length - 1; i > -1; i--) {
                ...
                that._removeConnectionsToTask(subTaskIndex, originalTasks, targetTasks);
                that._removeConnection(subTaskIndex);
                that._refreshTaskResources(originalTasks[subTaskIndex]);
        ...
        that.$.taskTimelineCellsContainer.removeChild(that.$.taskTimelineCellsContainer.children[index]);

        //Remove the task from the Task Tree
        that._removeTreeItem(index);
        ...
        if (selectedIndex) {
            that._select('task', that._getItemByTreeIndex('task', selectedIndex));
        }
    ...
    */
    ...
    _checkTaskBarResizability(event) {
        ...
        if (timelineTaskCell.classList.contains('smart-timeline-task-cell')) {
            ...
            timelineTaskCell = that.$.taskTimelineCellsContainer.children[that._tasks.indexOf(timelineTaskCell._task)];
        }
        ...
        function configureWindow() {
            ...
            if (that.popupWindowCustomizationFunction) {
                that.popupWindowCustomizationFunction(popupWindow, type,
                    target.classList.contains('smart-task-connection') ? target.getAttribute('connection-id') : that._tasks.indexOf(target._task));
            }
    ...
    _popupWindowClosingHandler(event) {
        ...
        const windowType = event.target.classList.contains('smart-confirm-popup-window') ? 'confirm' :
            (event.target.classList.contains('smart-connection-popup-window') ? 'connection' : 'task');
    ...
    _popupWindowTransitionendHandler(event) {
        ...
        if (targetWindow instanceof Smart.Window && !targetWindow.opened && event.propertyName === 'visibility') {
    ...
    _createTaskEditor(column, task, editorContainers) {
        ...
        if (editorContainer) {
            ...
            if (typeof value === 'number' || typeof value === 'string') {
                ...
                if (Smart.DropDownList) {
                    editElement = document.createElement('smart-drop-down-list');
    ...
    */
    _setAriaControls(type) {
        ...
        for (let i = 0; i < treeSplitterItems.length; i++) {
            ...
            if (that.$.taskTreeSplitterItem === splitterItem) {
                ...
                const dataContainer = splitterItem.querySelector(`.smart-${type}-tree-content`);

                if (!dataContainer) {
                    continue;
                }

                items = dataContainer.querySelectorAll(`.smart-${type}-label-container`);
            }
    ...
    _setPopupWindowEditors(column, targetTask, editorContainer) {
        ...
        if (!column.customEditor) {
            ...
            if (typeof value === 'number') {
                ...
                if (Smart.DropDownList && editElement instanceof Smart.DropDownList) {
        ...
        for (let i = 0; i < treeSplitterItems.length; i++) {
            ...
            if (splitterItem.contains(tree)) {
                ...
                if (insertAsLastItem) {
                    ...
                    tree.insert(item, typeof index === 'number' ? tree.querySelectorAll('smart-tree-item, smart-tree-items-group')[index] : index);
                }
            }
            else {
                const dataContainer = splitterItem.getElementsByClassName(`smart-${itemType}-tree-content`)[0],
                ...
                if (insertAsLastItem) {
                    targetColumnItem = dataContainer.getElementsByClassName(`smart-${itemType}-item`)[index];
    ...
    _ensureVisible(item, type) {
        ...
        if (item instanceof HTMLElement) {
            tree = item.menu || item.closest('smart-tree');
            treeItem = item;
        }
        ...
        if (tree) {
            const treeItems = Array.from(tree.querySelectorAll('smart-tree-item, smart-tree-items-group'));
    ...
    _handleExpanding(projectTask, expand) {
        ...
        for (let t = 0; t < projectTasks.length; t++) {
            ...
            if (expand) {
                ...
                that.$.container.style.setProperty('--smart-gantt-chart-task-timeline-content-height', timelineContentHeight + 'px');
    ...
    _hoverViaKeyboardCallback(hoveredTreeitem) {
        ...
        const tree = hoveredTreeitem.menu || hoveredTreeitem.closest('smart-splitter');
        ...
        const itemIndex = Array.from(tree.querySelectorAll('smart-tree-item, smart-tree-items-group')).indexOf(hoveredTreeitem),
        ...
        if (!aninationContainer.classList.contains('smart-visibility-hidden')) {
            if (itemName === 'task') {
                const animationTasks = aninationContainer.getElementsByClassName(`smart-timeline-${itemName}`);
        ...
        for (let i = 0; i < taskSplitterItems.length; i++) {
            ...
            if (taskSplitterItem !== that.$[itemName + 'TreeSplitterItem']) {
                //Unfocus last item
                const item = taskSplitterItem.getElementsByClassName(`smart-${itemName}-label-container`)[itemIndex];
        ...
        function unfocusTreeItems(treeSplitter) {
            ...
            for (let i = 0; i < splitterItems.length; i++) {
                ...
                if (splitterItem !== that.$[itemName + 'TreeSplitterItem']) {
                    const items = splitterItem.getElementsByClassName(`smart-${itemName}-label-container`);
    ...
    _getItemByTreeIndex(itemType, index, onlyProjects) {
        ...
        let targetProject = that[`_${itemType}s`].filter(item => !item.hidden && !item.project)[index.split('.')[0]];
    ...
    _taskTreeChangeHandler(event) {
        ...
        if (that[`_${treeType}APIManipulation`] || (that._itemClickedDetails && that._itemClickedDetails.type === treeType) || that._dragDetails) {
            return;
        }
        ...
        if (isResourceViewChange) {
            ...
            for (let t = 0; t < tasks.length; t++) {
                ...
                if (that.selectedIndexes.includes(t)) {
                    that.$.taskTimelineCellsContainer.children[t].setAttribute('selected', '');
                }
    ...
    _prepareViewTasks(isInit) {
        ...
        function reCreateTaskColumnsData() {
            ...
            if (taskTreeSplitterItems.length !== columnHeadersDetails.length) {
                ...
                for (let i = 0; i < columnHeadersDetails.length; i++) {
                    that._createTreeColumnsData(taskTreeSplitterItems[i].closest('smart-splitter-item'), columnHeadersDetails[i], treeTasks);
                }
            }
        }
        ...
        for (let i = 0; i < resIds.length; i++) {
            ...
            if (resId !== 'unassigned') {
                ...
                resourceLabel = resourceLabel.sort((a, b) => a.localeCompare(b)).join(', ').trim();
            }
            ...
            resourceViewTasks[i].connections = resourceViewTasks[i].connections.filter(con => {
                ...
                if (resourceViewTaskIndex < 0) {
                    resourceViewTasks[i]._connections ?
                        resourceViewTasks[i]._connections.push(Object.assing({}, con)) : resourceViewTasks[i]._connections = [Object.assign({}, con)];
                    return false;
                }
    ...
    */
    ...
    _downHandler(event) {
        ...
        if (target.closest('.smart-timeline-content') || target.closest(`.smart-${targetType}-tree-content`)) {
        ...
        if (!timelineTaskCell) {
            ...
            if (!target.closest('.smart-tree-item-label-container') && !target.closest('smart-scroll-bar')) {
                ...
                if (index !== undefined) {
                    that._itemClickedDetails = {
                        index: index,
                        type: targetType
                    };

                    that._select(targetType, that[`_${targetType}s`][that._itemClickedDetails.index]);
                }
                else {
                    const timelineResource = target.closest('.smart-timeline-resource-cell');
        ...
        if (target.closest('.smart-resource-timeline-view-cell') || that.disabled) {
            return;
        }
        ...
        if (timelineTask.getElementsByClassName('smart-timeline-task-fill')[0] || that.$.timeline.hasAttribute('task-bar-hovered')) {
            ...
            if (taskBarThumb || (!taskBarThumb && task.type === 'project' && task.synchronized && !task.dragProject)) {
                if (target.closest('.smart-task-connection-point')) {
    ...
    */
    _documentUpHandler(event) {
        ...
        if (that._itemClickedDetails !== undefined) {
            that._itemClickedDetails = undefined;
            return;
        }
        ...
        if (targetTaskBar) {
            targetTask = that.$.taskTimelineCellsContainer.children[that._tasks.indexOf(targetTaskBar._task)];
        }
    ...
    _getTreeItemIndex(target) {
        ...
        let item = target.closest('.smart-task-label-container') || target.closest('.smart-resource-label-container'), itemType;

        if (item) {
            itemType = item.classList.contains('smart-resource-label-container') ? 'resource' : 'task';

            return [].slice.call(item.closest('smart-splitter-item').getElementsByClassName(`smart-${itemType}-label-container`)).indexOf(item);
        }

        item = target.closest('.smart-tree-item-label-container');

        if (item) {
            return [].slice.call(item.closest('smart-tree').getElementsByClassName('smart-tree-item-label-container')).indexOf(item);
        }
    }
    ...
    _handleTreeItemHover(index, target) {
        ...
        const labelContainers = that.$.mainSplitter.querySelectorAll('smart-tree-item[hover], smart-tree-items-group[hover],' +
            ' .smart-task-label-container[hover], .smart-resource-label-container[hover]');
        ...
        if (Smart.Utilities.Core.isMobile || !that[itemType + 'Columns'].length) {
            return;
        }
        ...
        for (let i = 0; i < taskSplitterItems.length; i++) {
            ...
            if (taskSplitterItem === that.$[itemType + 'TreeSplitterItem']) {
                ...
                items = [].slice.call(taskSplitterItem.getElementsByClassName(`smart-${itemType}-label-container`));
            }
    ...
    */
    _getSubTaskLimits(timelineTask) {
        ...
        if (lastSubTask) {
            ...
            if (timelineSubTask) {
                const timelineTaskCell = that.$.taskTimelineCellsContainer.children[that._tasks.indexOf(lastSubTask)];
    ...
    _getTaskBarDetails(taskBar) {
        ...
        const timelineTask = that.$.taskTimelineCellsContainer.children[that._tasks.indexOf(taskBar._task)];
    ...
    _getTimelineTaskCellByDate(task, date) {
        ...
        if (task.classList && task.classList.contains('smart-timeline-task')) {
            task = that.$.taskTimelineCellsContainer.children[that._tasks.indexOf(task._task)];
        }
    ...
    _createResourceTimeline() {
        ...
        for (let i = 0; i < resources.length; i++) {
            ...
            cellsContainer.classList.add('smart-timeline-resource-cell');
        ...
        that.$.container.style.setProperty('--smart-gantt-chart-resource-timeline-content-height', that.$.resourceTimelineCellsContainer.offsetHeight + 'px');
    ...
    _refreshResourceTimelineContent(resource, timelineResource) {
        ...
        for (let i = 0; i < timelineResourceCells.length; i++) {
            ...
            cell.classList.add('smart-resource-timeline-view-cell');
    ...
    _refreshResourceScrollBar() {
        ...
        const newScrollHeight = that.$.resourceTimelineCellsContainer.offsetHeight - that.$.resourceTimelineContent.offsetHeight;
        ...
        if (!that.$.container.classList.contains('vscroll') && resourceVScroll.disabled) {
            resourceVScroll.classList.add('smart-hidden');
        }
        else {
            resourceVScroll.classList.remove('smart-hidden');
    ...
    */
    ...
    insertResource(index, resource) {
        ...
        let targetItem, insertAsLastItem;
        ...
        if (typeof index === 'number' && !isNaN(index)) {
            ...
            if (!targetItem) {
                ...
                if (targetItem) {
                    insertAsLastItem = index !== that._getItemPath(targetItem, 'resource');
                }
            }
        }
        else if (typeof index === 'object') {
        ...
        const originalResources = resources.slice(0);
        ...
        //Inserts the resource to the Timeline
        if (that.$.mainSplitter.contains(that.$.resourceSplitter) && !resource.hidden) {
            //Find the previous visible item
            if (targetItem && targetItem.hidden) {
                for (let i = index; i < originalResources.length; i++) {
                    const res = originalResources[i];
            ...
            that._insertTimelineResource(originalResources.filter(r => !r.hidden).indexOf(targetItem), resource, insertAsLastItem);
        }
    }
    ...
    _insertTimelineResource(index, resource, insertAsLastItem) {
        ...
        that.$.container.style.setProperty('--smart-gantt-chart-resource-timeline-content-height', resourceCellsContainer.offsetHeight + 'px');
    ...
    updateResource(resourceId, settings) {
        ...
        if (typeof resourceId === 'number' && !isNaN(resourceId)) {
            ...
            if (!resource) {
                resource = that._getItemByTreeIndex('resource', resourceId);
            }
        }
        else if (typeof resourceId === 'object') {
            resource = resources[that._getItemIndex(resourceId, 'resource')]
        }
    ...
    _getTaskResources(taskId, original) {
        ...
        if (typeof taskId === 'string') {
            ...
            task = tasks[that._getItemIndex(taskId, 'task')];
        }
        ...
        let resources = [],
            visibleResources = that._resources.filter(r => !r.hidden);
    ...
    _resourceValidator(oldValue, newValue) {
        ...
        if (Array.isArray(newValue) || newValue instanceof Smart.ObservableArray || typeof newValue === 'number' ||
    ...
    _handleTaskProgressChange(event) {
        ...
        const targetTaskProgressElement = targetTask.querySelector('.smart-timeline-task-progress'),
        ...
        const task = that._dragDetails.timelineTask._task;
    ...
    */
    _handleTaskBarDrag(event) {
        ...
        const targetTaskBar = that._dragDetails.timelineTask,
            timelineTask = that.$.taskTimelineCellsContainer.children[that._tasks.indexOf(targetTaskBar._task)];
    ...
    */
    ...
    _handleTimelineHover(event, reset) {
        ...
        function resetHoverState() {
            if (that._hoveredTimelineTask) {
                const targetTask = that._tasks.indexOf(that._hoveredTimelineTask._task);
        ...
        const timelineTaskIndex = that._tasks.indexOf(timelineTask._task),
        ...
        if (timelineTask) {
            ...
            that._handleTreeItemHover(that._tasks.indexOf(timelineTask._task), target);
        }
    }
    ...
    _handleResourceTimelineHover(target, reset) {
        ...
        let timelineResource = target.closest('.smart-timeline-resource-cell');
    ...
    _refreshTreeSplitterScrollTop(splitter, scrollTop) {
        ...
        for (let i = 0; i < treeColumns.length; i++) {
            ...
            if (column === that.$[itemName + 'TreeSplitterItem']) {
                ...
                const content = column.getElementsByClassName(`smart-${itemName}-tree-content`)[0];
    ...
    */
    ...
    _refreshTask(taskBar) {
        ...
        const task = taskBar._task,
            taskIndex = that._tasks.indexOf(task),
            timelineTaskElement = that.$.taskTimelineCellsContainer.children[taskIndex];
    ...
    */
    ...
    _refreshTreeColumnsData(item, properties, type) {
        ...
        const itemIndex = that[`_${type}s`].indexOf(item);
        let columnIndexes = [];
        ...
        for (let i = 0; i < columnIndexes.length; i++) {
            ...
            if (column === that.$[`${type}TreeSplitterItem`]) {
                taskItem = column.querySelectorAll('smart-tree-item, smart-tree-items-group')[itemIndex];
                ...
                taskItem = column.getElementsByClassName(`smart-${type}-label-container`)[itemIndex];
    ...
    _removeTreeItem(index, itemType = 'task') {
        ...
        for (let i = 0; i < treeSplitterItem.length; i++) {
            ...
            if (splitterItem.contains(tree)) {
                tree.removeItem(tree.querySelectorAll('smart-tree-item, smart-tree-items-group')[index]);
            }
            else {
                const dataContainer = splitterItem.getElementsByClassName(`smart-${itemType}-tree-content`)[0],
                    headerDetails = that[`${itemType}Columns`][i];
                ...
                const targetTask = dataContainer.getElementsByClassName(`smart-${itemType}-item`)[index];
    ...
    */
    _removeConnection(connectionId, noAutoScheduling) {
        ...
        const connections = connectionContainer.querySelectorAll('.smart-task-connection[connection-id^="' + connectionId + (connectionId.indexOf('-') < 0 ? '-' : '') + '"]'),
            tasks = that._tasks,
            timelineTasks = that.$.timelineTasksContainer.children,
    ...
    */
    _select(type = 'task', item, noScrolling) {
        ...
        const itemIndex = that[`_${type}s`].filter(i => !i.hidden).indexOf(item);
        ...
        that.$[`${type}Tree`].select(that.$[`${type}Tree`].querySelectorAll('smart-tree-item, smart-tree-items-group')[itemIndex]);
        ...
        for (let i = 0; i < splitterItems.length; i++) {
            ...
            items = [].slice.call(splitterItem.getElementsByClassName(`smart-${type}-label-container`));
        ...
        if (oldSelection[0] !== itemIndex) {
            that.selectedIndexes = [itemIndex];

            if (!that._noChangeEvent) {
                that.$.fireEvent('change', { value: [itemIndex], oldValue: oldSelection });
            }
        }
    }
    ...
    _highlightAssignedItem(type, index) {
        ...
        highlightedItems = Array.from(that.$.mainSplitter.
            querySelectorAll(`.smart-timeline-${highlightType}-cell[assigned], .smart-${highlightType}-label-container[assigned], 
                                  .smart-timeline-${highlightType}[assigned]`)).
            concat(Array.from(treeSplitterItem.querySelectorAll('.smart-tree-item[assigned], smart-tree-items-group[assigned]')));
        ...
        const item = items.filter(i => !i.hidden)[index];
        ...
        for (let i = 0; i < splitterItems.length; i++) {
            ...
            if (splitterItem === treeSplitterItem) {
                items = splitterItem.querySelectorAll('smart-tree-item, smart-tree-items-group');
            }
            else {
                items = splitterItem.getElementsByClassName(`smart-${highlightType}-label-container`);
            }
    ...
    */
    _setScrollBars() {
        ...
        resVScrollBar.$.addClass('smart-hidden');
    ...
    */
    _setTaskBarLabel(task, label) {
        ...
        taskLabel.innerHTML = task.formatFunction ? (task.formatFunction(task.label, that._tasks.indexOf(task)) || '') : (label || task.label);
    }
    ...
    _unselectAll(itemType, noEventFiring) {
        ...
        const allTimelineCells = timelineCellsContainer.querySelectorAll(`.smart-timeline-${itemType}-cell[selected]`);
        ...
        if (itemType === 'task') {
            const allTimelineTasks = that.$.timelineTasksContainer.querySelectorAll('.smart-timeline-task[selected]');
        ...
        const labelContainers = that.$[`${itemType}TreeSplitter`].getElementsByClassName(`smart-${itemType}-label-container`);
    ...
    */
     ...
     */
    _getTasksJSON(isResourceView) {
        const that = this;
        let allTasks = that._tasks,
            projectName = 'project';

        if (isResourceView) {
            projectName = '_project';
            allTasks = that.__tasks;
        }
        ...
        function cloneObject(obj) {
            ...
            delete newObj.project;
            delete newObj._project;
        ...
        function getTasksJSON(tasks, project) {
            ...
            for (let t = 0; t < tasks.length; t++) {
                ...
                if ((!task[projectName] && !project) || task[projectName] === project) {
                    ...
                    if (isResourceView) {
                        if (!task.disableResources) {
                            clonedObject.connections = clonedObject.connections.map(connection => Object.assign({}, connection));
    ...
    _createTreeColumns(splitter) {
        const that = this,
            ...
            columnHeaders = splitter.getElementsByClassName(`smart-${itemName}-tree-header`);
        ...
        if (columnHeadersDetails.length > columnHeaders.length) {
            while (columnHeadersDetails.length > columnHeaders.length) {
                ...
                splitterItem.innerHTML = `<div class="smart-${itemName}-tree-header"></div><div class="smart-${itemName}-tree-content">`;
            ...
            while (columnHeaders.length > columnHeadersDetails.length) {
                ...
                splitter.removeChild(splitter.lastElementChild);
            }
        }
        ...
        const parentSplitter = splitter.closest('smart-splitter');
    ...
    _refreshColumnsHeaders(type) {
        ...
        const columnHeadersDetails = that[`${type}Columns`],
            columnHeaders = that.$[`${type}TreeSplitter`].getElementsByClassName(`smart-${type}-tree-header`);
        ...
        if (!arguments.length && that.$.mainSplitter.contains(that.$.resourceSplitter)) {
            that._refreshColumnsHeaders('resources');
        }
    }
    ...
    _createTreeColumnsData(splitterItem, headerDetails, items, itemName) {
        ...
        const value = headerDetails.value,
            tree = splitterItem.querySelector('smart-tree');
        ...
        if (tree) {
            if (value) {
                ...
                tree.dataSource = items.map((item) => prepareTaskData(item, true));
        ...
        const dataContainer = splitterItem.getElementsByClassName(`smart-${itemName}-tree-content`)[0];
    ...
    * @param {any} tasks - header Details
    ...
    _createTreeItemContainers(items, headerDetails, itemName) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            taskContainer.innerHTML = `<div class="smart-${itemName}-label-container"></div>`;
            ...
            if (item.tasks && item.tasks.length > 0) {
                ...
                taskContainer.innerHTML += `<div class="smart-${itemName}-drop-down ${!item.expanded ? 'smart-visibility-hidden' : ''}"></div>`;
                taskContainer.lastElementChild.appendChild(that._createTreeItemContainers(item.tasks, headerDetails, itemName));
            }

            taskContainer.classList.add(`smart-${itemName}-item`);
    ...
    _formatColumnData(item, headerDetails, noEllipsisWrapper) {
        const that = this,
            type = that.taskColumns.includes(headerDetails) ? 'task' : 'resource';
            ...
            if (projectTasks) {
                for (let i = 0; i < projectTasks.length; i++) {
                    if (projectTasks[i].project === project && projectTasks[i][dateName]) {
                        taskDate = new Date(Math[operation](projectTasks[i][dateName].getTime(), taskDate.getTime()))
                    }
                }
            }
    ...
    _mouseWheelandler(event) {
        const that = this,
            popupWindow = (that.shadowRoot || that).querySelector('.smart-task-popup-window');
        ...
        if (!that.disabled) {
            if (that.$.resourceSplitterItem.contains(event.target)) {
                const scrollBar = that.$.resourceVerticalScrollBar;

                if (scrollBar.classList.contains('smart-hidden')) {
                    return;
                }
                ...
                if (scrollTop === 0 && event.deltaY < 0 ||
                    scrollTop === scrollBar.max && event.deltaY > 0) {
                    event.stopPropagation();
                    return;
                }
                ...
                that._resourceScrollView.scrollTo(scrollTop + that._getScrollCoefficient(event, that.offsetHeight));
                return;
            }

            if (that.$.taskSplitterItem.contains(event.target) && that.computedVerticalScrollBarVisibility) {
                ...
                that.scrollTo(scrollTop + that._getScrollCoefficient(event, that.offsetHeight));
            }
        }
    }
