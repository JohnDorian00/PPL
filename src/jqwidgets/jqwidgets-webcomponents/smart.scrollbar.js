...
Smart('smart-scroll-bar', class ScrollBar extends Smart.BaseElement {
    ...
    _farButtonClickHandler() {
        ...
        that._updateValue(that.value + (that.orientation === 'horizontal' && that.rightToLeft ? -1 : 1) * that.step);
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            ...
            case 'showButtons': {
                ...
                if (propertyName === 'min') {
                    that.setAttribute('aria-valuemin', newValue);
                }
                else if (propertyName === 'max') {
                    that.setAttribute('aria-valuemax', newValue);
                }
                else if (propertyName === 'orientation') {
                    that.setAttribute('aria-orientation', newValue);
                }
    ...
    render() {
        ...
        that.setAttribute('role', 'scrollbar');
        ...
        that.setAttribute('aria-valuenow', that.value);
    ...
    _trackClickHandler(event) {
        ...
        const value = (that.rightToLeft ? -1 : 1) * that.value;

        if (that.orientation === 'horizontal') {
            if (x > (that._isThumbDragged ? that.dragStartX : thumbRect.right)) {
                that._updateValue(that.value + (that.rightToLeft ? -1 : 1) * that.largeStep);
            }
    ...
    _updateValue(oldValue, newValue) {
        ...
        if (oldValue !== newValue) {
            that.setAttribute('aria-valuenow', newValue);
