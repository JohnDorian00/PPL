...
Smart('smart-tab-item', class TabItem extends Smart.ContentElement {
    ...
    render() {
        ...
        that.firstElementChild.setAttribute('role', 'presentation');
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            ...
            case 'selected':
                ...
                if (that.tabLabelContainer) {
                    that.tabLabelContainer.setAttribute('aria-' + propertyName, newValue);
                }
...
Smart('smart-tabs', class Tabs extends Smart.BaseElement {
    ...
    static get listeners() {
        return {
            ...
            'tabStrip.touchmove': '_tabStripTouchmoveHandler',
            'tabStrip.touchstart': '_tabStripTouchstartHandler',
    ...
    appendChild(node) {
        ...
        if (!that.isRendered) {
            that.whenRendered(() => that.insert(that._tabs.length, { node: node }));
        }
        else {
    ...
    ensureVisible(index) {
        ...
        if (that.tabPosition === 'top' || that.tabPosition === 'bottom') {
            if (tabStripBoundingRect.left > scrollTargetBoundingRect.left) {
                tabStrip.scrollLeft += Math.round(scrollTargetBoundingRect.left - tabStripBoundingRect.left);
            }
            else {
                tabStrip.scrollLeft += Math.round(scrollTargetBoundingRect.right - tabStripBoundingRect.right);
            }
        }
            ...
            tabStrip.scrollLeft = that._getScrollLeft(0);
    ...
    insert(index, details) {
        ...
        that._setAriaRelations(tab, tabLabelContainer, dropDownLabelContainer);
    ...
    refreshTabHeader() {
        ...
        that._positionTabSelectionBar(that._tabLabelContainers ? that._tabLabelContainers[that.selectedIndex] : undefined, true);
    }
    ...
    _createElement() {
        ...
        if (!that.$.dropDownButton.id) {
            that.$.dropDownButton.id = that.id + 'DropDownButton';
        }

        if (!that.$.dropDownButtonDropDown.id) {
            that.$.dropDownButtonDropDown.id = that.id + 'DropDown';
        }

        that.setAttribute('role', 'tablist');
        that.$.dropDownButton.setAttribute('role', 'button');
        that.$.dropDownButton.setAttribute('aria-haspopup', true);
        that.$.dropDownButton.setAttribute('aria-owns', that.$.dropDownButtonDropDown.id);
        that.$.dropDownButtonDropDown.setAttribute('role', 'menu');
        ...
        if (that.rightToLeft) {
            that.$.tabsHeaderItems.removeChild(that.$.dropDownButton);

            if (that.tabPosition === 'top' || that.tabPosition === 'bottom') {
                that.$.tabsHeaderItems.insertBefore(that.$.dropDownButton, that.$.scrollButtonNear);
            }
            else {
                that.$.tabsHeaderItems.insertBefore(that.$.dropDownButton, that.$.tabSelectionBar);
            }
        }
        ...
        if (that.enableShadowDOM) {
            HTMLElement.prototype.appendChild.apply(that, [that.$.hiddenInput]);
        }
    }
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            ...
            case 'overflow':
                ...
                that.$.tabStrip.scrollLeft = that._getScrollLeft(0);
                that.$.tabStrip.scrollTop = 0;
            ...
            case 'rightToLeft':
                ...
                if (newValue && (that.tabPosition === 'top' || that.tabPosition === 'bottom')) {
                    that.$.tabsHeaderItems.insertBefore(that.$.dropDownButton, that.$.scrollButtonNear);
                }
                else {
                    that.$.tabsHeaderItems.insertBefore(that.$.dropDownButton, that.$.tabSelectionBar);
    ...
    _addGroupContainer(label, labelSize) {
        ...
        groupContainer.setAttribute('aria-expanded', false);
        groupContainer.setAttribute('aria-haspopup', true);
        groupContainer.setAttribute('aria-label', label);
        labelTextWrapper.className = 'smart-tab-label-text-wrapper';
        labelTextWrapper.setAttribute('role', 'presentation');
        labelTextWrapper.setAttribute('aria-hidden', true);
        ...
        dropDown.id = that.id + 'GroupDropDown' + label;
        dropDown.setAttribute('role', 'tablist');
        dropDown.setAttribute('aria-orientation', 'vertical');
        groupContainer.setAttribute('aria-owns', dropDown.id);
    ...
    _addTabLabelContainer(tab, addNewTab) {
        ...
        // label
        labelTextContainer.className = 'smart-tab-label-text-container';
        labelTextContainer.setAttribute('role', 'presentation');
    ...
    _closeDropDownButtonDropDown() {
        ...
        if (that._dropDownButtonDropDownOpened) {
            that.$.dropDownButton.setAttribute('aria-expanded', false);
    ...
    _closeGroupDropDown() {
        ...
        const group = that._openDropDown.groupContainer;
    ...
    _documentUpHandler(event) {
        ...
        if (!that.$.dropDownButton.contains(target) && event.originalEvent.type !== 'pointercancel') {
            that._closeDropDownButtonDropDown();
        }
    ...
    _dropDownButtonHandler(target, event, dropDown) {
                ...
                switch (eventType) {
                    ...
                    case 'up':
                        if (target === that._downTarget &&
                            event.originalEvent && event.originalEvent.type !== 'pointercancel') {
        ...
        if (eventType === 'down') {
            ...
            if (!that._dropDownButtonDropDownOpened) {
                ...
                that.$.dropDownButton.setAttribute('aria-expanded', true);
    ...
    _endSwiping(event, now) {
        ...
        const scrollable = function () {
            if (that.rightToLeft && orientationSettings.scrollDirection === 'scrollLeft') {
                const scrollLeft = Math.abs(that._getScrollLeft());

                if (distanceDifference < 0 &&
                    scrollLeft === tabStrip[orientationSettings.scrollSize] - tabStrip[orientationSettings.size] ||
                    distanceDifference > 0 && scrollLeft === 0) {
                    return false;
                }
            }
            else {
            ...
            if (that.rightToLeft && orientationSettings.scrollDirection === 'scrollLeft') {
                if (Smart.Utilities.Core.Browser.Chrome) {
                    ...
                    if (distanceDifference > 0) {
                        ...
                        tabStrip[orientationSettings.scrollDirection] = Math.min(oldScrollDirection - scrollStep, 0);
                    }
                }
            }
            else {
    ...
    _handlePosition(newPosition, oldPosition) {
        ...
        if (newPosition === 'hidden') {
            ...
            that.setAttribute('aria-orientation', 'horizontal');
            return;
        }
    ...
    _keydownHandler(event) {
        ...
        let indexToSelect, noSelect, keyPrevious, keyNext, step = 0;

        if (that.rightToLeft) {
            keyPrevious = 'ArrowRight';
            keyNext = 'ArrowLeft';
        }
        else {
            keyPrevious = 'ArrowLeft';
            keyNext = 'ArrowRight';
        }
    ...
    */
    ...
    /**
     * Sets WAI-ARIA relations between tab and tabpanel elements.
     */
    _setAriaRelations(tab, tabLabelContainer, dropDownLabelContainer) {
        const that = this,
            tabsId = that.id;

        if (!tab.id) {
            tab.id = tabsId + 'TabItem' + tab.index;
        }

        if (!tabLabelContainer.id) {
            tabLabelContainer.id = tabsId + 'TabLabelContainer' + tab.index;
        }

        tabLabelContainer.setAttribute('role', 'tab');
        tabLabelContainer.setAttribute('aria-controls', tab.id);
        tabLabelContainer.setAttribute('aria-label', tab.label);
        tabLabelContainer.setAttribute('aria-selected', tab.selected);
        tab.setAttribute('aria-label', tab.label);
        dropDownLabelContainer.setAttribute('role', 'menuitem');
        dropDownLabelContainer.setAttribute('aria-controls', tab.id);
    }
    ...
    _positionTabSelectionBar(selectedTabLabelContainer, noAnimation) {
        ...
        if (tabPosition === 'top' || tabPosition === 'bottom') {
            ...
            if (that.rightToLeft && that.tabTextOrientation === 'horizontal') {
                if (Smart.Utilities.Core.Browser.Chrome) {
                    const tabStrip = that.$.tabStrip;

                    left = -(tabStrip.scrollLeft - (tabStrip.scrollWidth - tabStrip.offsetWidth)) + selectedTabLabelContainer.offsetLeft + 'px';
                }
                else {
    ...
    _positionGroupDropDown(groupLabel) {
        const that = this,
            ...
            headerSectionPaddingLeft = parseFloat(headerSectionStyle[that.rightToLeft ? 'paddingRight' : 'paddingLeft']),
        ...
        if (position === 'top' || position === 'bottom') {
            ...
            if (that.rightToLeft) {
                dropDown.style.left = groupLabel.offsetLeft - that._getScrollLeft() - headerSectionPaddingLeft - dropDown.offsetWidth + groupLabel.offsetWidth + 'px';
            }
            else {
    ...
    /**
    * Returns the scrollLeft of the TabStrip
    */
    _getScrollLeft(scrollLeft) {
        ...
        if (scrollLeft === undefined) {
            if (that.rightToLeft && Smart.Utilities.Core.Browser.Chrome) {
                return tabStrip.scrollLeft - (tabStrip.scrollWidth - tabStrip.offsetWidth);
            }
            else {
                return tabStrip.scrollLeft;
            }
        }

        if (!that.rightToLeft || (that.tabPosition !== 'top' && that.tabPosition !== 'bottom')) {
            return scrollLeft;
        }

        if (Smart.Utilities.Core.Browser.Chrome) {
            scrollLeft = (tabStrip.scrollWidth - tabStrip.offsetWidth) - scrollLeft;
        }
        else {
            scrollLeft *= -1;
        }
    ...
    _tabsHeaderItemsWheelHandler(event) {
        ...
        if (!that.enableMouseWheelAction || that.disabled) {
            return;
        }

        const tabStrip = that.$.tabStrip,
            orientationSettings = that._orientationSettings,
            size = tabStrip[orientationSettings.size],
            scrollSize = tabStrip[orientationSettings.scrollSize],
            scrollDirection = tabStrip[orientationSettings.scrollDirection],
            delta = event.deltaY;

        if (size === scrollSize ||
            scrollDirection === 0 && delta < 0 ||
            scrollDirection + size === scrollSize && delta > 0) {
            return;
        }
    ...
    _tabStripHandler(target, event) {
        ...
        if (closestLabelContainer !== null) {
            if (event.originalEvent && event.originalEvent.type === 'pointercancel') {
                if (closestLabelContainer.tab.group) {
                    delete closestLabelContainer.parentElement.rippleInProgress;
                }
            }
            else {
    ...
    /**
     * tabStrip touchmove handler.
     */
    _tabStripTouchmoveHandler(event) {
        const that = this;
        ...
        const touches = event.touches[0],
            scrollDirection = tabStrip[orientationSettings.scrollDirection],
            coord = touches[orientationSettings.coordinate],
            normalizedCoord = parseFloat(coord.toFixed(5)),
            previousCoord = coords[orientationSettings.coordinate === 'pageY' ? 1 : 0],
            normalizedPreviousCoord = parseFloat(previousCoord.toFixed(5));

        that._touchCoords = [touches.pageX, touches.pageY];

        if (scrollDirection === 0 && normalizedCoord >= normalizedPreviousCoord || // pan up
            scrollDirection + size === scrollSize && normalizedCoord <= normalizedPreviousCoord) { // pan down
            return;
        }
     ...
     * tabStrip touchmove handler.
     */
    _tabStripTouchstartHandler(event) {
        const touches = event.touches[0];
    ...
    _updateScrollButtonVisibility(noTabSelectionBarAnimation) {
        ...
        if (that.rightToLeft && orientationSettings.scrollDirection === 'scrollLeft') {
            ...
            if (Math.round(tabStrip[orientationSettings.size] + scrollLeft) >= Math.round(tabStrip[orientationSettings.scrollSize]) - 1) {
                showNear = false;
            }
        }
        else {
