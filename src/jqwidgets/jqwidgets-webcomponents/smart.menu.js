...
Smart('smart-menu-item', class MenuItem extends Smart.BaseElement {
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        if (that.tagName === 'Smart-TREE-ITEM') {
            return;
        }
...
Smart('smart-menu-items-group', class MenuItemsGroup extends Smart.BaseElement {
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        if (that.tagName === 'Smart-TREE-ITEMS-GROUP') {
            return;
        }
...
Smart('smart-menu', class Menu extends Smart.BaseElement {
    ...
    attached() {
        ...
        if (that.isCompleted && that.isRendered && that.dropDownAppendTo !== null) {
            if (that._minimized) {
    ...
    _createItemHTMLStructure(currentItem, level, item, index) {
        ...
        let currentItemPath, parentObject;
        ...
        labelContainer.className = 'smart-' + that._element + '-item-label-container';
        labelElement.className = 'smart-' + that._element + '-item-label-element';
    ...
    _createMenuItemsGroupContainer(item, level) {
        ...
        itemContainer.className = 'smart-' + that._element + '-item-container';
        ...
        if (item.dropDownHeight !== null) {
            const repeatButtonNear = document.createElement('smart-repeat-button'),
    ...
    _closeOnDocumentInteraction(event) {
        ...
        if (that.mode === 'dropDown' && that.opened) {
            ...
            if (!isInsideElement && (!closestDropDown || closestDropDown.ownerElement !== that)) {
                that.close({ trigger: 'interaction', target: Smart.EnableShadowDOM ? shadowRootTarget : target });
                return true;
            }
        }
    ...
    _ensureVisible(scrollTarget) {
        ...
        function scrollTo(scrollDimension, dimension) {
            ...
            if (scrollDimension === 'Top' && parent !== that.$.mainContainer && !scrollButtons[0].$.hasClass('smart-hidden')) {
    ...
    _handleOverflowChange() {
        ...
        if ((that._minimized || that.dropDownAppendTo === null && !that.$mainContainer.hasClass('simple') && that.mode !== 'tree') && !(Smart.ListMenu && that instanceof Smart.ListMenu)) {
            return;
        }
    ...
    _wheelHandler(event) {
        ...
        if (!that.enableMouseWheelAction || that.disabled || that.displayLoadingIndicator) {
            return;
        }
