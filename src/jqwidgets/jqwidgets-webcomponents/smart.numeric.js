...
Smart.Utilities.Assign('NumberRenderer', class NumberRenderer {
    ...
    toScientific() {
        ...
        if (that.numericValue._d) {
            ...
            if (that.numericValue <= -1 || that.numericValue >= 1) {
                exponentialValue = Number(that.toDigits(arguments[0], arguments[1])).toExponential();
            }
            else {
                exponentialValue = Number(that.numericValue).toExponential();
            }
        }
        ...
        let power = parseInt(exponentialValue.slice(indexOfE + 1), 10),
            coefficient = parseFloat(exponentialValue.slice(0, indexOfE)),
            coefficientRenderer = new Smart.Utilities.NumberRenderer(coefficient);

        if (power < 0) {
            let test = coefficientRenderer.toDigits(arguments[0], arguments[1]);

            test = parseFloat(test).toExponential();

            if (test.charAt(test.length - 1) === '0') {
                let i = 0;
                ...
                if (i) {
                    ...
                    Smart.Utilities.BigNumber.ignoreBigIntNativeSupport = true;
                    coefficient = parseFloat(new Smart.Utilities.BigNumber(coefficient).multiply(Math.pow(10, i)).toString());
                    Smart.Utilities.BigNumber.ignoreBigIntNativeSupport = ignored;
                }

                coefficientRenderer.numericValue = coefficient;
                coefficient = coefficientRenderer.toDigits(arguments[0], arguments[1]);

                // test for alternative, more concise representation
                coefficientRenderer.numericValue = coefficient / 1000;

                const alternativeCoefficient = coefficientRenderer.toDigits(arguments[0], arguments[1]);

                if (alternativeCoefficient >= 1) {
                    coefficient = alternativeCoefficient;
                    power += 3;
                }

                return coefficient + that.powersToPrefixes[power.toString()];
            }
        }

        let remainderPower = 0;

        if (Math.abs(power) > 24) {
            const sign = power / Math.abs(power);

            remainderPower = power - sign * 24;
        }
        else {
            remainderPower = power % 3;
        }
        ...
        let oldCoefficientPower = Number(coefficient).toExponential();

        oldCoefficientPower = parseFloat(oldCoefficientPower.charAt(oldCoefficientPower.length - 1));

        coefficientRenderer.numericValue = coefficient;
        coefficient = coefficientRenderer.toDigits(arguments[0], arguments[1]);

        let newCoefficientPower = Number(coefficient).toExponential(),
            powerCorrection = 0;

        newCoefficientPower = parseFloat(newCoefficientPower.charAt(newCoefficientPower.length - 1));

        if (newCoefficientPower !== oldCoefficientPower && newCoefficientPower % 3 === 0) {
            coefficient = parseFloat(coefficient) / (Math.pow(10, newCoefficientPower));
            coefficientRenderer.numericValue = coefficient;
            coefficient = coefficientRenderer.toDigits(arguments[0], arguments[1]);
            powerCorrection = newCoefficientPower;
        }

        const finalPower = power - remainderPower + powerCorrection,
