    ...
    _keyDownHandler(event) {
        ...
        switch (event.key) {
            ...
            case 'PageDown':
                ...
                break;
            case 'Enter': {
                const focusedItem = typeof that.$.listBox._focusedItem === 'function' ? that.$.listBox._focusedItem() : that.$.listBox._focusedItem;

                if (that.opened && focusedItem && focusedItem._focused) {
                    that.$.listBox.$.filterInput.value = focusedItem[that.inputMember];
                    that.$.input.value = that.displayMode === 'escaped' ?
                        that._toEscapedDisplayMode(focusedItem[that.inputMember]) : that._toDefaultDisplayMode(focusedItem[that.inputMember]);
   ...
   */
    _listBoxChangeHandler(event) {
        ...
        if (event.detail.selected) {
            const selectedItem = that.$.listBox._items[event.detail.index];
            let label = '';

            if (selectedItem) {
                label = selectedItem[that.inputMember];
            }
