...
Smart('smart-list-item', class Item extends Smart.ContentElement {
    ...
    set _focused(value) {
        ...
        if (value) {
            ...
            if (that.ownerListBox && that.ownerListBox.ownerElement && Smart.ComboBox &&
                that.ownerListBox.ownerElement instanceof Smart.ComboBox) {
                that.ownerListBox.ownerElement.$.input.setAttribute('aria-activedescendant', that.id);
            }
        }
        else {
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        if (propertyName === 'hidden') {
            ...
            if (newValue) {
                ...
                if (that.readonly || that.disabled || !listBox ||
                    ['oneOrManyExtended', 'zeroOrMany', 'oneOrMany', 'checkBox'].indexOf(listBox.selectionMode) === -1) {
...
Smart('smart-list-items-group', class ListItemsGroup extends Smart.ContentElement {
    ...
    */
    ...
    attached() {
        ...
        if (!that.$.itemsLabel.id) {
            that.$.itemsLabel.id = that.id + 'Label';
        }

        that.setAttribute('aria-labelledby', that.$.itemsLabel.id);
    }
});
/**
...
Smart('smart-list-box', class ListBox extends Smart.ScrollViewer {
    ...
    */
    ...
    get value() {
        ...
        if (that.isRendered && that._selectedItems.length > 0) {
            return that._selectedItems.map(item => {
    ...
    set value(newValue) {
        ...
        const item = that.getItem(newValue);
    ...
    _downHandler(event) {
        ...
        if (!Smart.ListBox.DragDrop.Feedback && that.allowDrag) {
            ...
            if (that.rightToLeft) {
                Smart.ListBox.DragDrop.Feedback.setAttribute('right-to-left', true);
            }
            else {
                Smart.ListBox.DragDrop.Feedback.removeAttribute('right-to-left');
            }
    ...
    _dblclickEventHandler(event) {
        ...
        that.$.editInput.dir = that.rightToLeft ? 'rtl' : 'ltr';
        that.$.editInput.style.width = that.$.itemsInnerContainer.offsetWidth + 'px';
        that.$.editInput.owner = target;
        that.$.editInput.value = target.label;
        that.$.editInput.style.height = target.offsetHeight + 'px';
        that.$.editInput.style.top = (target.offsetTop + that.$.itemsInnerContainer.offsetTop) + 'px';
        that.$.editInput.setAttribute('aria-label', 'Edit item ' + target.label);

        if (that.$editInput.hasClass('smart-hidden')) {
            that.$editInput.removeClass('smart-hidden');
        }

        that.$.editInput.focus({ preventScroll: true });
    }
        ...
        if (Smart.ListBox.DragDrop.Feedback) {
            ...
            if (Smart.Utilities.Core.isMobile && Smart.ListBox.DragDrop.Dragging) {
                for (let j = 0; j < Smart.ListBox.DragDrop.ListBoxes.length; j++) {
                    ...
                    if (rect.left <= event.clientX && event.clientX <= rect.left + rect.width && rect.top <= event.clientY && event.clientY <= rect.top + rect.height) {
                        dropTarget = listBox;
                        ...
                        for (let i = 0; i < visibleItems.length; i++) {
                            ...
                            if (visibleItems.indexOf(item) === visibleItems.length - 1 || item.parentNode.lastElementChild === item) {
                                if (event.clientY >= rect.top + item.offsetTop - listBox.scrollTop + (itemRect.height / 2) && event.clientY <= rect.top - listBox.scrollTop + item.offsetTop + itemRect.height) {
                                    Smart.ListBox.DragDrop.DropDetails = { item: item, position: 'bottom' };
                                    break;
                                }
                            }
                            if (event.clientY >= rect.top + item.offsetTop - listBox.scrollTop && event.clientY <= rect.top + item.offsetTop + itemRect.height - listBox.scrollTop) {
    ...
    _touchmoveHandler(event) {
        if (Smart.ListBox.DragDrop.Dragging && event.cancelable) {
            event.preventDefault();
            event.stopPropagation();
            return;
        }

        super._touchmoveHandler(event);
    }
   ...
   */
    ...
    */
    ...
    render() {
        ...
        that.setAttribute('role', 'listbox');
    ...
    attached() {
        ...
        if (that.selectedIndexes.length > 0 && that._items) {
    ...
    */
    select(item) {
        ...
        if (item.grouped || item.readonly || !that._items.length || (item instanceof HTMLElement && !that.contains(item))) {
            return;
        }
    ...
    _filterItems(noSelectionRefresh, callback, ignoreFilterQueryLength) {
        ...
        if (typeof (that.dataSource) === 'function') {
            if (filterQuery || ignoreFilterQueryLength) {
                ...
                //Notify that the DataSource has changed
                that.$.fireEvent('bindingComplete');
            }
        }
        else {
    ...
    */
    get _scrollWidth() {
        ...
        that.__scrollWidth = (() => {
            const that = this,
                virtualized = that.virtualized,
                itemMeasureMode = that.itemMeasureMode;
            let longestItemWidth = 0;

            let longestLabel = '';

            function measure(labelToMeasure) {
                const measureItem = that._createItem({ label: labelToMeasure });

                //NOTE: Avoids resize event being thrown as a result of element height/width change
                measureItem.style.position = 'absolute';

                that.$.itemsContainer.appendChild(measureItem);
                measureItem.label = labelToMeasure;
                ...
                const padding = parseInt(measureItemStyle.paddingLeft) + parseInt(measureItemStyle.paddingRight),
                    measureItemContent = measureItem.$.itemContainer || measureItem,
                    result = measureItemContent.offsetWidth + measureItemContent.offsetLeft + padding;
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                if (itemMeasureMode === 'auto') {
                    const label = virtualized ? currentItem.label : currentItem.textContent.trim();
                    ...
                    longestItemWidth = Math.max(longestItemWidth, measure(currentItem.label));
                }
            }

            if (itemMeasureMode === 'auto') {
                return measure(longestLabel);
            }
    ...
    insert(index, items) {
        ...
        else if (that._updateListBoxAfterQuery) {
            that._updateListBoxAfterQuery();
            delete that._updateListBoxAfterQuery;
        }
    }
    ...
    */
    _handleKeyStrokes(key) {
        ...
        if (that.disabled || that.readonly || !items.length) {
            return;
        }
    ...
    */
    _horizontalScrollbarHandler(event) {
        ...
        if (that.isVirtualized) {
            ...
            that.$.itemsContainer.scrollLeft = that._getScrollLeft(event.detail.value);
        }
    }
    ...
    _getScrollLeft(scrollLeft, scrollWidth) {
        ...
        if (Smart.Utilities.Core.Browser.Chrome) {
            if (!scrollWidth) {
                ...
                scrollWidth = that._scrollWidth - that.$.itemsContainer.offsetWidth - horizontalOffset;
            }
    ...
    _keyUpHandler(event) {
        ...
        if (that.filterable && that.$.filterInput === activeElement) {
            ...
            that._filterTimer = setTimeout(function () {
                ...
                that.scrollLeft = that.$.itemsContainer.scrollLeft = that._getScrollLeft(0);
                that.context = context;
            }, 300);
            return;
        }
    ...
    */
    _refreshLayout() {
        ...
        //Reset the width before measure
        that.$.itemsInnerContainer.style.width = '';
    ...
    */
    _refreshHorizontalScrollBarVisibility(scrollWidth) {
        ...
        if (that.computedHorizontalScrollBarVisibility) {
            that.scrollLeft = that._getScrollLeft(that.$.itemsContainer.scrollLeft, scrollWidth);
    ...
    */
    ...
    _recycle() {
        ...
        if (itemIndex >= 0) {
            ...
            for (let i = itemIndex; i < totalItems; i++) {
                const item = items[i];
                ...
                if (itemTop + item.height >= value && itemTop <= value + height) {
                    ...
                    listItem.setAttribute('aria-setsize', totalItems);
                    listItem.setAttribute('aria-posinset', i);
    ...
    */
    ...
    _applyAriaSelected() {
        const that = this,
            items = that.isVirtualized ? that._recyclingItems : that._items,
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            else if (item.readonly || item.disabled ||
                ['oneOrManyExtended', 'zeroOrMany', 'oneOrMany', 'checkBox'].indexOf(selectionMode) === -1) {
