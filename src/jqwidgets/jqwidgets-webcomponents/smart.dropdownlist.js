...
Smart('smart-drop-down-list', class DropDownList extends Smart.ContentElement {
    ...
    */
    _applySelection() {
        ...
        if (that.selectionDisplayMode === 'plain') {
            ...
            that.$.actionButton.innerHTML = !newValue.length ? '' : `<span class="smart-token">${newValue.join(', ').trim()}</span>`;
            return;
        }
    ...
    */
    _applyTokenTemplate(label, icon, buttonLabel) {
        ...
        const element = document.createElement('span'),
            token = function () {
                return '<span class=\'smart-drop-down-list-selection-label\' role="presentation">' + label +
                    `</span><span class=\'smart-drop-down-list-unselect-button\' role="button"${buttonLabel ? ' aria-label="Unselect"' : ''}>${icon}</span>`;
            };
    ...
    */
    _buttonsDownHandler(event) {
        ...
        if (that.tagName.toLowerCase().indexOf('smart-drop-down-') > -1 && that.dropDownOpenMode === 'dropDownButton' && event.target === that.$.actionButton) {
    ...
    */
    _documentUpHandler(event) {
        ...
        if (isActionButtonPressed === that.$.actionButton || target.closest('.smart-drop-down-button') === that.$.dropDownButton) {
            ...
            that.$dropDownContainer.hasClass('smart-visibility-hidden') && that.dropDownOpenMode !== 'none' &&
    ...
    */
    ...
    _blurEventHandler() {
        ...
        if (that.nodeName && that.nodeName.toLowerCase() === 'smart-drop-down-list' && !that._preventDropDownClose) {
            that.close();
        }
    }
    ...
    */
    _mouseEnterHandler() {
        ...
        if (that.tagName.toLowerCase().indexOf('smart-drop-down-') > -1 && that.dropDownOpenMode === 'auto' && !that.disabled && !that.readonly) {
            that.open();
        }
    }
