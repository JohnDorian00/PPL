        ...
        switch (propertyName) {
            ...
            case 'expandMode':
                ...
                if (newValue === 'multiple' || newValue === 'toggle') {
                    that._items[that.expandedIndexes[0]].$.label.removeAttribute('aria-disabled');
                }
                else {
                    that._items[that.expandedIndexes[0]].$.label.setAttribute('aria-disabled', true);
                }
   ...
   */
    ...
    */
    _createElement() {
        ...
        for (let i = 0; i < that._items.length; i++) {
            that._items[i].setAttribute('part', 'smart-accordion-item');
   ...
   */
    ...
    */
    _keyDownHandler(event) {
        ...
        if (['ArrowLeft', 'ArrowDown', 'ArrowRight', 'ArrowUp', 'End', 'Home', ' ', 'Enter', 'Tab'].indexOf(keyCode) === -1 || !(activeElement instanceof Smart.AccordionItem)) {
            return;
        }
    ...
    */
    _moveHandler(event) {
        ...
        if (!that._itemIsPressed || !that.reorder || that.readonly || that._items.length < 2 ||
            (!that.$container.hasClass('smart-reordering') && Math.abs(event.pageY - that._pointerPosition) < 5)) {
            return;
        }

        if (that._dragStart) {
            if (!that.$container.hasClass('smart-reordering')) {
            ...
            if (that.hasAnimation) {
                const ripple = that._selectedItem.querySelector('.smart-ripple');
