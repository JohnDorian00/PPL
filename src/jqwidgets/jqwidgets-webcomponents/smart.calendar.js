...
Smart('smart-calendar', class Calendar extends Smart.BaseElement {
    ...
    navigate(step) {
        ...
        if (typeof step === 'number') {
            ...
            if (that.weeks !== 6 && that.weeks !== 0 && that.displayMode === 'month') {
                ...
                const visibleWeeks = [].slice.call(that._focusedCell.closest('.smart-calendar-weeks').children).filter(week => !week.classList.contains('smart-hidden'));
    ...
    */
    _applySelectionMode(initial, oldSelectionMode) {
        ...
        if (['default', 'many', 'zeroOrMany', 'oneOrMany', 'week', 'range'].indexOf(selectionMode) !== -1) {
            Array.from(that.getElementsByClassName('smart-calendar-month')).forEach(month => month.setAttribute('aria-multiselectable', true));
        }
        else {
            Array.from(that.getElementsByClassName('smart-calendar-month')).forEach(month => month.removeAttribute('aria-multiselectable'));
        }
    ...
    */
    _downHandler(event) {
        ...
        if (Smart.Utilities.Core.isMobile && event.originalEvent.target.closest('.smart-calendar-body') === that.$.body) {
        ...
        if (that.displayModeView === 'list' && that.displayMode !== 'month') {
            target = (that.shadowRoot || that.isInShadowDOM ? event.originalEvent.composedPath()[0] : event.originalEvent.target).closest('smart-list-item');
    ...
    _documentUpHandler(event) {
        ...
        if (!Smart.Utilities.Core.isMobile || !that._dragStartDetails ||
            event.originalEvent.target.closest('.smart-calendar-list-view')) {
            return;
        }
        ...
        if (rightToLeft) {
            for (let w = 0; w < weeks.length; w++) {
                ...
                for (let d = weekDays.length - 1; d > 0; d--) {
                    if (weekDays[d].classList.contains('smart-calendar-cell')) {
    ...
    _setDisplayMode(oldValue, newValue, animationTarget) {
        ...
        if ((oldValue === 'month' && newValue === 'year') || (oldValue === 'year' && newValue === 'decade')) {
            ...
            if (that.displayModeView === 'list') {
                ...
                target = (that.$.listViewContainer.shadowRoot ? that.$.listViewContainer.shadowRoot : that.$.listViewContainer).querySelector('smart-list-item[selected]');
                that.$.animationPanel.style.transformOrigin = (target.offsetLeft + target.offsetWidth / 2) + 'px ' +
                    (target.dataItem.top > 0 ? target.offsetTop - target.offsetHeight / 2 : target.offsetTop + target.offsetHeight / 2) + 'px';
            }
            else {
    ...
    _isDateInView(date) {
        ...
        for (let d = 0; d < that._viewDates.length; d++) {
            if (date.getFullYear() === that._viewDates[d].getFullYear() && date.getMonth() === that._viewDates[d].getMonth()) {
                ...
                return cell.otherMonth || !cell.offsetHeight || cell.classList.contains('smart-visibility-hidden') ? false : true;
            }
        }
    ...
    _setScrollButtonsNavigationMode() {
        ...
        if (that.scrollButtonsNavigationMode !== 'landscape') {
            if (that.$.previousMonthButton) {
                that.$.previousMonthButton.$.button.firstElementChild.className = 'smart-arrow-down smart-arrow';
            }

            if (that.$.nextMonthButton) {
                that.$.nextMonthButton.$.button.firstElementChild.className = 'smart-arrow-up smart-arrow';
            }
        }
        else {
            if (that.$.previousMonthButton) {
                that.$.previousMonthButton.$.button.firstElementChild.className = 'smart-arrow-left smart-arrow';
            }

            if (that.$.nextMonthButton) {
                that.$.nextMonthButton.$.button.firstElementChild.className = 'smart-arrow-right smart-arrow';
            }
        }
    ...
    _updateWeeksVisibility(step, cell) {
        ...
        function futureWeeks() {
            for (let i = weekIndex; i < monthWeeks.length; i++) {
                if (!monthWeeks[i].classList.contains('smart-hidden') || shouldWeekBeHidden(monthWeeks[i])) {
                    continue;
                }
        ...
        //NOTE: classlist is used because when months > 1(or animation is ON) all weeks need to extend Smart
        if (!cell && !focusedWeek.classList.contains('smart-hidden')) {
            return;
        }
