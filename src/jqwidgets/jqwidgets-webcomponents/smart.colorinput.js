...
Smart('smart-color-input', class ColorInput extends Smart.Input {
                        ...
                        <div class="smart-buttons-container" role="presentation" id="buttonsContainer">
                            <div class="smart-action-button" id="actionButton">
                                ...
                                <input class="smart-input smart-color-input" id='input' readonly='[[readonly]]' placeholder='[[placeholder]]' type='[[type]]' name='[[name]]' 
                                    value='{{value::keyup}}' disabled='[[disabled]]' aria-label="[[placeholder]]" spellcheck="false" />
                            </div>
    ...
    _blurHandler() {
        ...
        if (!that._isValidColor(that.value)) {
            that.$.input.value = that.$.input.dataValue = that.value = '';
    ...
    _formatValue(value) {
        ...
        const color = that._getRGBA(value);
  ...
  */
    ...
    _inputSelectHandler() {
        ...
        if (that.readonly) {
            that.$.input.selectionStart = that.$.input.selectionEnd;
        }
    }
    ...
    _keyUpHandler(event) {
        ...
        switch (event.keyCode) {
            ...
            default:
                if (that.readonly) {
                    if (event.keyCode === 32 && !that.opened) {
                        that.open();
                    }
    ...
    _createElement() {
        ...
        scrollView.setAttribute('theme', that.theme);
